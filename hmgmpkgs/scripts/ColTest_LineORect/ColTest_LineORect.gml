function ColTest_LineORect(state_machine_id) : RenderState(state_machine_id) constructor {
	s1 = noone;
	s2 = noone;
	static on_enter = function() {
		...
		s2.shape = oriented_rectangle_create(s2.id,100,0,64,66,20);
	}
	...
	static process_input = function() {
		if (keyboard_check_pressed(vk_space)) state_machine.change("seg-circle");
	}
	...
	static update = function(dt) {
		...
		if (collide_line_in_oriented_rectangle(s1.shape,s2.shape)) {
	...
	static draw = function() {
		draw_text(1,1,"Line-Oriented Rectangle Collisions");
