function ColTest_LineLineSegment(state_machine_id) : RenderState(state_machine_id) constructor {
	s1 = noone;
	s2 = noone;
	static on_enter = function() {
		...
		s2.shape = line_segment_create(s2.id,0,0,64,66);
	}
	...
	static process_input = function() {
		if (keyboard_check_pressed(vk_space)) state_machine.change("line-orect");
	}
	...
	static update = function(dt) {
		...
		if (collide_line_in_line_segment(s1.shape,s2.shape)) {
	...
	static draw = function() {
		draw_text(1,1,"Line-Line Segment Collisions");
