...
function InputManager() constructor {
	...
	//static save_settings_to_file = function(path) {
	//	if (!file_exists(path)) show_error(path + " could not be found.",true);
	//}
	
	static load_settings_from_file = function(path) {
		if (!file_exists(path)) show_error(path + " could not be found.",true);
		...
		ds_map_clear(axis_map);
		
		var json_action_map = json_map[? "actions" ];
		var json_action_count = ds_map_size(json_action_map);
		var json_action_map_key = ds_map_find_first(json_action_map);
		for (var i = 0; i < json_action_count; ++i) {
			// name of action is json_action_map_key
			// data of action is json_action which is a list of data for ActionInputs
			var json_action = json_action_map[? json_action_map_key ];
			var json_action_input_count = ds_list_size(json_action);
			for (var j = 0; j < json_action_input_count; ++j) {
				var json_action_input = json_action[| j ];
				var type = json_action_input[? "type" ];
				var input = json_action_input[? "input" ];
				type = type_conversion_map[? type ];
				input = input_conversion_map[? input ];
				add_action_input(json_action_map_key,type,input);
			}
			json_action_map_key = ds_map_find_next(json_action_map, json_action_map_key);
		}
		
		var json_axes = json_map[? "axes" ];
